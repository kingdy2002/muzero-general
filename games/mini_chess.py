...
class Minichess:
            ...
            for action in can_action :
                ...
                if peice_id == 1 :
                    if action in [28,29] :
                        if self.peice_loc[destination[0]][destination[1]] * self.player >= 0 :
                            continue
                life_peice , white_peice_loc , black_peice_loc = self.apply_action(destination[0]+destination[1]*6+36*action)
                if (self.is_mate(life_peice , white_peice_loc , black_peice_loc )[0 if self.player == 1 else 1] > 0 ) :
                    continue
    ...
    def is_mate(self,life_peice = None, white_peice_loc = None, black_peice_loc = None):
        ...
        for peice in white_life_peice :
            ...
            can_action = self.peice.peice_can_action(peice_id)
            for action in can_action :
                move = self.action2move(action,player=1)
                destination = (loc[0]+move[0], loc[1]+move[1])
